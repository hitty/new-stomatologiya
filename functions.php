...
function yourdent_filter_doctors() {
	...
	if ( $category ) {
		$meta_query[] = [ 'key' => 'doc_category', 'value' => $category, 'compare' => 'LIKE' ];
	}
	if ( $clinic ) {
		$meta_query[] = [ 'key' => 'doc_clinic', 'value' => $clinic, 'compare' => 'LIKE' ];
	}
	...
	if ( $query->have_posts() ) {
		while ( $query->have_posts() ) {
            ...
            <a href="<?php the_permalink(); ?>" class="all-doctor-card">
				<?php if ( $photo ) : ?>
                    <img src="<?= esc_url( $photo['url'] ); ?>" alt="<?= esc_attr( $name ); ?>"
                         class="all-doctor-photo">
				<?php endif; ?>
